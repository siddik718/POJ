...
export const statistics = async (req, res) => {
  ...
  try {
    ...
    const RE = submissions.filter((submission) => {
      return (submission.status !== "Compilation Error" && submission.status !== "Accepted" && submission.status !== "Wrong Answer" && submission.status !== "Time Limit Exceeded");
    });
    ...
    const JAVA = submissions.filter((submission) => {
      return submission.language === "JAVA";
    }); 
